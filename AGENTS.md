...
The MCP server is configured in `opencode.json` for OpenCode and `.mcp.json` for Claude Code. It starts automatically when you open the project.

### Quick Start

1. Install dependencies: `bun install` (from project root)
2. Open the project — approve the MCP server when prompted
